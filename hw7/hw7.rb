...
class Point < GeometryValue
  ...
  # shape s and the seg.  seg is an instance of LineSegment
  def intersectWithSegmentAsLineResult seg
      if (seg.x1 < @x and @x < seg.x2 and 
         (seg.y1 < @y and @y < seg.y2 or
          seg.y2 < @y and @y < seg.y1))
          self
      else
          NoPoints.new()
      end
...
class Line < GeometryValue
  ...
  def intersectPoint p
    if real_close_point(p.x,p.y,p.x,@m*p.x + @b)
      Point.new(p.x,p.y)
    else 
      NoPoints.new()
    end
